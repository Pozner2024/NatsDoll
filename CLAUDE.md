...
| Слой        | Технология                                    |
| ----------- | --------------------------------------------- |
| Frontend    | Nuxt 4 (Vue 3, SSR) + Pinia + TypeScript + SCSS |
...
| Рассылка    | Resend (email-верификация, newsletter)        |
...
Исключение: если имя главного компонента конфликтует со страницей из `src/pages`, допускается суффикс `Widget` (`CartPageWidget.vue`, `ProductPageWidget.vue`).
...
**Z-index:** каскадом через `calc(var(--предыдущий) + 1)`, все значения в `global.scss` (блок `--z-*`).
...
Кодовая база большая — tsc/vitest запускать с увеличенным heap. Корневой `tsc --noEmit` НЕ работает (project references, TS6306) — api чекать через `-p apps/api`:
