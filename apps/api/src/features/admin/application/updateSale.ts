...
import type { AdminRepository, UpdateSale } from '../types'

function hasOverlap(aStart: Date, aEnd: Date, bStart: Date, bEnd: Date): boolean {
  return aStart <= bEnd && aEnd >= bStart
}

export function makeUpdateSale(repo: AdminRepository): UpdateSale {
  return async (id, input) => {
    ...
    const conflict = existing.find(
      (s) => s.id !== id && hasOverlap(newStart, newEnd, new Date(s.startsAt), new Date(s.endsAt)),
    )
    if (conflict) {
      throw new AppError(409, `Sale period overlaps with existing sale "${conflict.name}"`)
    }

    await repo.updateSale(id, input)
  }
}
