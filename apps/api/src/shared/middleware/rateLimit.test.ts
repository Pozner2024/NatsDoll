...
describe('createRateLimiter', () => {
  ...
  it('берёт последний IP из x-forwarded-for (тот что добавил nginx)', async () => {
    const { app } = buildApp(1, 60_000)
    // Атакующий шлёт `x-forwarded-for: 5.5.5.5`, nginx добавляет свой IP справа: `5.5.5.5, 10.0.0.1`.
    // Лимитер должен использовать `10.0.0.1`, а не подделанное значение клиента.
    expect((await app.request('/', { headers: { 'x-forwarded-for': '5.5.5.5, 10.0.0.1' } })).status).toBe(200)
    expect((await app.request('/', { headers: { 'x-forwarded-for': '99.99.99.99, 10.0.0.1' } })).status).toBe(429)
  })

  it('игнорирует подделанный x-forwarded-for когда есть x-real-ip', async () => {
    const { app } = buildApp(1, 60_000)
    expect((await app.request('/', { headers: { 'x-forwarded-for': '5.5.5.5', 'x-real-ip': '10.0.0.1' } })).status).toBe(200)
    expect((await app.request('/', { headers: { 'x-forwarded-for': '6.6.6.6', 'x-real-ip': '10.0.0.1' } })).status).toBe(429)
  })
  ...
  it('использует x-real-ip когда нет x-forwarded-for', async () => {
